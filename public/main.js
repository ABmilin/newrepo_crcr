...
async function updateDueDate(id, due_date) {
  ...
  if (window.calendar) {
    ...
    if (event) {
      event.setStart(due_date);
    } else {
      // イベントが見つからない場合（例: 新規追加）、再読み込み
      loadCalendar?.(); // 安全に呼び出し
    }
  }
}
...
async function loadRecommendations(reportId) {
  ...
  // ✅ ここで配列かどうかをチェック（安全に .map() を使うため）
  if (!Array.isArray(recommendations)) {
    console.error("類似案件の形式が不正です:", recommendations);
    container.innerHTML = "<p style='color:red;'>類似案件の取得に失敗しました</p>";
    return;
  }
    ...
    <div class="recommendations-list">
      ...
      <ul>
        ${recommendations.map(r => `
          <li>
            <strong>${r.title}</strong>（スコア: ${r.score || "?"}）
            <br><small>理由: ${r.reason || "N/A"}</small>
          </li>
        `).join("")}
      </ul>
    </div>
