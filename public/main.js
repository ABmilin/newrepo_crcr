...
async function loadReports() {
  ...
  container.innerHTML = reports.map(r => `
    <article>
      ...
      <strong>期限:</strong>
      <input type="date" value="${r.due_date ? r.due_date.split('T')[0] : ""}" onchange="handleDueDateChange('${r.id}', this.value)">
...
async function handleDueDateChange(id, newDueDate) {
  await updateDueDate(id, newDueDate);
  if (typeof loadCalendarEvents === "function") {
    loadCalendarEvents(); // カレンダーに即時反映
  }
}
