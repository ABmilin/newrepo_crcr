...
async function loadReports() {
  ...
  container.innerHTML = reports.map(r => {
    ...
    const inquiry = lines.find(l => l.includes("問い合わせ内容")) || "不明";
    const urgency = lines.find(l => l.includes("緊急度")) || "未設定";
    const actions = lines.filter(l => l.includes("・") || l.includes("方針") || l.includes("対策"));

    // --- 緊急度の色分けクラス ---
    let urgencyClass = "urgency-low";
    if (urgency.includes("高")) urgencyClass = "urgency-high";
    else if (urgency.includes("中")) urgencyClass = "urgency-medium";
    ...
    return `
      <article>
        ...
        <p>${inquiry.replace(/^.*?:\s*/, "")}</p>

        <strong>緊急度:</strong>
        <p class="${urgencyClass}">${urgency.replace(/^.*?:\s*/, "")}</p>

        <strong>対応方針:</strong>
        <ul>
          ${actions.map(a => `<li>${a.replace(/^\*|\d\.|・/g,"").trim()}</li>`).join("")}
        </ul>
