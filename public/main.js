import { supabase } from "./supabaseClient.js"; // 追加
...
document.getElementById("reportForm").addEventListener("submit", async (e) => {
  ...
  const user_id = currentUser.id; // 実運用時はSupabase AuthのユーザーID
...
async function loadReports() {
  ...
  container.innerHTML = reports.map(r => `
    <article>
      ...
      <small>作成者: ${r.user_id} / 最終編集者: ${r.last_editor || "未編集"}</small>
...
async function editReport(id, title, content) {
  ...
  if (newTitle && newContent) {
    await fetch(`/api/report/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: newTitle, content: newContent, last_editor: currentUser.email })
    });
    loadReports();
  }
}
...
async function updateStatus(id, status) {
  await fetch(`/api/report/${id}/status`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ status, last_editor: currentUser.email })
  });
}
...
async function updateAssignee(id, assignee) {
  await fetch(`/api/report/${id}/assignee`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ assignee, last_editor: currentUser.email })
  });
}
...
async function updateDueDate(id, due_date) {
  await fetch(`/api/report/${id}/due_date`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ due_date, last_editor: currentUser.email })
  });
}
