...
router.get("/:reportId", async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: "スタンプの取得に失敗しました" });
  res.json(data);
});
...
router.post("/:reportId", async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: "スタンプの追加に失敗しました" });
  res.json(data[0]);
});
...
router.delete("/:reportId", async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: "スタンプの削除に失敗しました" });
