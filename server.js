...
app.post("/api/report", async (req, res) => {
  ...
  if (!title || !content || !user_id) {
    ...
    return res.status(400).json({ error: "タイトル・内容・ユーザーIDは必須です" });
  }
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("reports")
      .insert([{ title, content, summary, user_id }])
      .select("id, title, content, summary, status, assignee, due_date, created_at, user_id, last_editor");
    if (error) {
...
app.put("/api/report/:id", async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("reports")
    .update({ title, content, last_editor })
    .eq("id", id)
    .select("id, title, content, summary, status, assignee, due_date, created_at, user_id, last_editor");
  if (error) {
...
app.put("/api/report/:id/status", async (req, res) => {
  const { id } = req.params;
  const { status, last_editor } = req.body;
  const { data, error } = await supabase
    .from("reports")
    .update({ status, last_editor }) // last_editorを追加
    .eq("id", id)
    .select("id, title, content, summary, status, assignee, due_date, created_at, user_id, last_editor");
...
app.put("/api/report/:id/assignee", async (req, res) => {
  const { id } = req.params;
  const { assignee, last_editor } = req.body; // ← last_editor を追加で受け取る
  const { data, error } = await supabase
    .from("reports")
    .update({ assignee, last_editor }) // ← last_editor も更新
    .eq("id", id)
    .select("id, title, content, summary, status, assignee, due_date, created_at, user_id, last_editor"); // ← 作成者・最終編集者も返す
  if (error) {
...
app.put("/api/report/:id/due_date", async (req, res) => {
  const { id } = req.params;
  const { due_date, last_editor } = req.body; // last_editorも受け取る
  const { data, error } = await supabase
    .from("reports")
    .update({ due_date, last_editor }) // last_editorも更新
    .eq("id", id)
    .select("id, title, content, summary, status, assignee, due_date, created_at, user_id, last_editor");
...
app.get("/api/dashboard", async (req, res) => {
  const { status, urgency, period } = req.query;

  let query = supabase.from("reports") .select("status, summary, created_at, title, user_id, last_editor");
