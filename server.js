...
app.post("/api/report", async (req, res) => {
  console.log("POST /api/report called:", req.body);

  // ✅ 1. assignee と due_date を受け取るようにする
  const { user_id, title, content, assignee, due_date } = req.body;
  ...
  try {
    ...
    // ✅ 2. insert に assignee と due_date を追加する
    const { data, error } = await supabase
      .from("reports")
      .insert([{ title, content, summary, assignee, due_date }]) // ← ここが重要！
      .select();
    ...
    console.error("Gemini or server error:", err);
...
app.get("/api/assignees", async (req, res) => {
  ...
  // 重複除去＋空白除外
  const assignees = [...new Set(data.map(d => d.assignee).filter(Boolean))];
  res.json(assignees);
});
